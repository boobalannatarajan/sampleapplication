...
public class UserAuthenticationController {
	...
	static final long EXPIRATIONTIME = 600000; // For user valid session upto 10 mins (mentioned as milisec)
	...
	public ResponseEntity<User> createUser(@RequestBody User user) {
		ResponseEntity<User> output = null;
		try {
			userAuthenticationService.saveUser(user);
			output = new ResponseEntity<User>(user, HttpStatus.CREATED);// return as 201 http status code

		} catch (UserAlreadyExistsException e) {
			return new ResponseEntity<User>(HttpStatus.CONFLICT);// return as 409 http status code
		}
		return output;
	}
	 ...
	 * The username and password should be validated before proceeding ahead with
	 * JWT token generation. The user credentials will be validated against the
	 * database entries. The error should be return if validation is not successful.
	...
	public ResponseEntity<?> login(@RequestBody User user) throws ServletException {
		String jwtToken = "";
		try {
			jwtToken = getToken(user.getUserId(), user.getUserPassword());// call getToken from JWT
			loginMap.clear();
			loginMap.put("message", "user successfully logged in keepNote App");
	...
	public String getToken(String userId, String password) throws Exception {
		...
		String jwtToken = Jwts.builder().setSubject(userId).setIssuedAt(new Date())
				...
				.signWith(SignatureAlgorithm.HS256, "secretkey").compact();// SHA 256 used in token
	...
	public ResponseEntity<?> isAuthenticated(ServletRequest req) throws IOException, ServletException {
		...
		try {
			...
			return new ResponseEntity<>(loginMap, HttpStatus.OK);
		} catch (Exception e) {// for Invalid Token
			loginMap.clear();
